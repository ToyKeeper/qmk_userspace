...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
    ...
    switch(keycode) {
        ...
        // - full 65%/75% layout:
        //   - LGUI:  hold: LGUI  /  tap: Mouse left click
        //   - RGUI:  hold: RGUI  /  tap: MouseLock  /  tap-hold: momentary MouseLock
        // - Whitefox:  Same as 65%, except...
        ...
        case TK_LGUI:
            ...
            break;
        #endif  // !defined(USE_PALM) && defined(OLD_QMK)
        #ifdef USE_PALM
        case TK_RGUI:
        case TK_LGUI:
            {
                ...
                if (0 == record->tap.count) {  // RGUI on first hold
                    if (record->event.pressed)
                ...
                else if ((1 == record->tap.count) && record->event.pressed) {
                    // toggle one-shot Alt on 1st press
                    ...
                    return false;
                } else if (record->tap.count > 0) {  // momentary RALT
                    if (record->event.pressed) {
            ...
            break;
        #elif defined(USE_RGUI_2TAP_MOUSELOCK) // not defined(USE_PALM)
        // Whitefox has RGUI, RAlt, and MouseLock all on one key
        case TK_RGUI:
            switch (record->tap.count) {
                case 0:  // RGUI on first hold
                    layer_off(L_MOUSE);
                    if (record->event.pressed)
                    ...
                    return false;
                case 1:  // 1tap: toggle one-shot Alt, or turn off mouse layer
                    unregister_mods(MOD_BIT(KC_RGUI));
                    if (IS_LAYER_ON(L_MOUSE)) {
                        layer_off(L_MOUSE);
                    } else if (record->event.pressed) {
                        if (get_oneshot_mods() & (MOD_BIT(KC_LALT) | MOD_BIT(KC_RALT)))
                            del_oneshot_mods(MOD_BIT(KC_RALT));  // undo OSM(RALT) if tapped again
                        else
                            add_oneshot_mods(MOD_BIT(KC_RALT));  // do OSM(RALT) on tap
                        return false;
                    }
                    break;
                case 2:  // 2tap: MouseLock
                    ...
                    break;
                default:  // momentary Alt on later holds
                    layer_off(L_MOUSE);
                    del_oneshot_mods(MOD_BIT(KC_RALT));
                    unregister_mods(MOD_BIT(KC_RGUI));
                    if (record->event.pressed)
        ...
        case TK_RGUI:
            ...
            if (0 == record->tap.count) {
                ...
                return false;
            } else if (record->tap.count > 1) {  // allow momentary usage on later taps
                if (record->event.pressed)
                    layer_on(L_MOUSE);
                else
                    layer_off(L_MOUSE);
            ...
            return true;
        #endif  // normal / USE_PALM / USE_RGUI_2TAP_MOUSELOCK
